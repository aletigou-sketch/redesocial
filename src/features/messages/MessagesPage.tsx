    ...
    <div className="messages-shell card">
      ...
      <div className="messages-thread">
        {!selected ? <div className="messages-empty"><MessageCircle size={42} /><h2>Escolha uma conversa</h2><p>Seu histórico privado aparecerá aqui.</p></div> : <>
          <header className="thread-header"><div><strong>{selected.displayName}</strong><small>@{selected.username}</small></div><div className="thread-header-actions"><div className="thread-status"><span className={`presence-label is-${otherPresence}`}><i aria-hidden="true" />{otherPresence === 'online' ? 'Online' : otherPresence === 'away' ? 'Ausente' : 'Offline'}</span><span className={connected ? 'is-online' : ''}>{connected ? 'Mensagens conectadas' : 'Reconectando mensagens…'}</span>{!presence.connected && <small>Presença reconectando…</small>}</div><ModerationActions targetType="profile" targetId={selected.otherUserId} userId={selected.otherUserId} compact onBlocked={() => void loadConversations()} /><button className="icon-button call-start" disabled={Boolean(call)} onClick={() => void startCall({ conversationId: selected.id, userId: selected.otherUserId, displayName: selected.displayName, username: selected.username }, 'audio')} aria-label={`Ligar para ${selected.displayName}`} title="Iniciar chamada de áudio"><Phone size={19} /></button><button className="icon-button call-start" disabled={Boolean(call)} onClick={() => void startCall({ conversationId: selected.id, userId: selected.otherUserId, displayName: selected.displayName, username: selected.username }, 'video')} aria-label={`Iniciar videochamada com ${selected.displayName}`} title="Iniciar videochamada"><Video size={19} /></button></div></header>
